...
const logger = debug_1.default('qc:prices');
exports.oldGetPrices = async (connection, courses, countryCode, provinceCode, discountAll, options) => {
    ...
    if (helpers.gbpCountry(countryCode) && typeof (options === null || options === void 0 ? void 0 : options.discountGBP) !== 'undefined') {
        if (typeof options.discountSignatureGBP !== 'undefined') {
            ...
            if (!verify.verify(public_key_1.default, Buffer.from(options.discountSignatureGBP, 'base64'))) {
    ...
    ////////////////////////////////////////////////////////////////////////////////
    const freeCourses = []; // array of courses that should be free
    ...
    ////////////////////////////////////////////////////////////////////////////////
    let primaryCourse = '';
    let totalCost = big_js_1.Big(0);
    let secondaryDiscount = big_js_1.Big(0);
    ...
    if (typeof (options === null || options === void 0 ? void 0 : options.discount) !== 'undefined') {
    ...
    else if (typeof (options === null || options === void 0 ? void 0 : options.campaignId) !== 'undefined' && typeof (options === null || options === void 0 ? void 0 : options.discountCode) !== 'undefined') {
        ...
        let sql = `SELECT c.course_restriction_type, c.payment_plan, c.offer_type, c.bonus_title, c.bonus_html,
                 ...
                 LIMIT 1`;
        let campaigns = await connection.query(sql, [options.campaignId, options.discountCode]);
  ...
  WHERE
    r.campaign_id = ?`;
            const restrictions = await connection.query(sqlRestrictions, options.campaignId);
    ...
    if (courses.length) {
        if (typeof result.campaign !== 'undefined' && result.campaign.requirementsMet) {
            ...
            // the running totals
            let totalCampaignDiscountFull = big_js_1.Big(0);
            let totalCampaignDiscountAccelerated = big_js_1.Big(0);
            let totalCampaignDiscountPart = big_js_1.Big(0);
            for (const course of courses) {
                ...
                const cost = big_js_1.Big(c.baseCost).minus(c.secondaryDiscountAmount);
    ...
    ////////////////////////////////////////////////////////////////////////////////
    let totalDepositFull = big_js_1.Big(0);
    let totalDepositAccelerated = big_js_1.Big(0);
    let totalDepositPart = big_js_1.Big(0);
    let totalInstallmentSizePart = big_js_1.Big(0);
    let totalInstallmentSizeAccelerated = big_js_1.Big(0);
    for (const course of courses) {
        ...
        let installmentSizeAccelerated = big_js_1.Big(0);
        let depositAccelerated = big_js_1.Big(0);
 ...
 */
async function lookupPrice(connection, courseCode, countryCode, provinceCode) {
    let sql = null;
    let prices;
    const sqlStart = `
SELECT
