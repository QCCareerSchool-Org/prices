...
import { studentSupport100Codes, studentSupport150Codes, studentSupport50Codes } from './promoCodeCalculator';
...
export class DiscountApplicator {
  ...
  public applyPromoCodeDiscounts(): void {
    ...
    const doOneTimeDiscount = (coursePrice: CoursePrice): void => {
      ...
      if (this.promoCode === 'FOUNDATION200' && isEventFoundationCourse(coursePrice.code)) {
      ...
      if (this.promoCode === 'KIT200OFF' && isMakeupFoundationCourse(coursePrice.code)) {
        this.safeApply(coursePrice, Big(this.currency.code === 'GBP' ? 100 : 200));
        applied = true;
        return;
      }

      if ([ 'MASTERCLASS', 'SSMASTERCLASS' ].includes(this.promoCode ?? '') && this.coursePrices.some(c => c.code === 'I2')) {
        this.safeApply(coursePrice, Big(200));
        applied = true;
        return;
      }

      if (this.promoCode === 'MASTERCLASS150' && this.coursePrices.some(c => c.code === 'I2')) {
        this.safeApply(coursePrice, Big(150));
        applied = true;
        return;
      }

      if (this.promoCode === 'MZ100' && this.coursePrices.some(c => c.code === 'MZ')) {
    ...
    for (const coursePrice of this.coursePrices) {
      ...
      if (this.promoCode === 'SKINCARE100' && coursePrice.code === 'MZ') {
        this.safeApply(coursePrice, Big(100));
        continue;
      }

      // take a flat amount off MZ
      if ([ 'SKINCARE300', 'MASTER300' ].includes(this.promoCode ?? '') && coursePrice.code === 'MZ') {
        this.safeApply(coursePrice, Big(300));
        continue;
      }

      if (studentSupport50Codes.includes(this.promoCode ?? '')) {
        this.safeApply(coursePrice, Big(50));
        continue;
      }

      if (studentSupport100Codes.includes(this.promoCode ?? '')) {
        this.safeApply(coursePrice, Big(100));
        continue;
      }

      if (studentSupport150Codes.includes(this.promoCode ?? '')) {
        this.safeApply(coursePrice, Big(150));
        continue;
      }
  ...
  private getMultiCourseDiscountRateOverride(coursePrice: CoursePrice): Big | undefined {
    if ((this.promoCode === 'SKINCARE60' && coursePrice.code === 'SK' && this.coursePrices.find(c => c.code === 'MZ'))) {
      return Big(0.6);
    }

    if ([ 'SAVE60', 'PORTFOLIO60', 'QCLASHES60', 'COLORWHEEL60' ].includes(this.promoCode ?? '')) {
      return Big(0.6);
    }

    if (this.promoCode === 'LIVEEVENT60') {
      return Big(0.6);
    }

    if (this.promoCode === 'ORGANIZING60' && coursePrice.code === 'PO') {
      return Big(0.6);
    }

    if (this.promoCode === 'CORPORATE60' && coursePrice.code === 'CP') {
      return Big(0.6);
    }

    if (this.promoCode === 'STYLING60' && coursePrice.code === 'PF') {
      return Big(0.6);
    }

    if (this.promoCode === 'PORTDEV60' && coursePrice.code === 'PW') {
      return Big(0.6);
    }

    if (this.promoCode === 'DAYCARE60' && coursePrice.code === 'DD') {
      return Big(0.6);
    }

    if (this.promoCode === 'SFX60' && coursePrice.code === 'SF') {
      return Big(0.6);
    }

    if (this.promoCode === 'BUSINESS60' && (coursePrice.code === 'EB' || coursePrice.code === 'DB')) {
      return Big(0.6);
    }

    if (this.promoCode === 'TRAINING60' && (coursePrice.code === 'DT' || coursePrice.code === 'DC')) {
      return Big(0.6);
    }
  }
  ...
  private getExtraPromoDiscount(): Big | undefined {
    ...
    if ([ 'DESIGN100OFF', 'EVENT100OFF' ].includes(this.promoCode ?? '')) {
      return Big(this.currency.code === 'GBP' ? 75 : 100);
    }

    if ([ 'DESIGN200OFF', 'EVENT200OFF' ].includes(this.promoCode ?? '')) {
      return Big(this.currency.code === 'GBP' ? 150 : 200);
    }

    if ([ 'BOGO100', 'BOGOCATALYST100', '2SPECIALTY100', 'SPECIALTY100', 'PROFITPIVOT' ].includes(this.promoCode ?? '')) {
      return Big(100);
    }

    if ([ 'PORTFOLIO50', 'FANDECK50', 'BRUSHSET50' ].includes(this.promoCode ?? '')) {
      return Big(50);
    }
  }
